...
- [x] **Add NI cDAQ (compactDAQ) support** — nidaqmx-backed path added; chassis is enumerated as a single "device" whose channels span its slotted modules. Use `ni_backend='nidaqmx'` (default is still `'pydaqmx'` during the migration window).
- [ ] **Audit trigger & logging logic across hardware paths** — verify trigger, pre-trigger buffer, and stop conditions behave the same for soundcard (`Recorder`) and NI (`Recorder_NI_PyDAQmx` / `Recorder_NI_nidaqmx`). Document actual behaviour.
- [x] **Evaluate driver choice for NI hardware** — nidaqmx added as an alternative backend to PyDAQmx (v1.1.0). Decision to fully remove PyDAQmx deferred until one clean hardware session on the nidaqmx path covers USB-6003, USB-6212, and cDAQ.
- [ ] **Remove PyDAQmx path** — once the nidaqmx backend has had one clean hardware session across USB-6003, USB-6212 and the cDAQ chassis (including shared-clock AI/AO sync), delete `Recorder_NI_PyDAQmx`, `setup_output_NI_pydaqmx`, the PyDAQmx import from [streams.py](pydvma/streams.py), the `ni_backend` setting from [options.py](pydvma/options.py), and flip the default in any docs. Keep the `Recorder_NI` alias pointing at the nidaqmx class for one more release to soften external imports.

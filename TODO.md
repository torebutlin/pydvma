...
- [ ] **Propagate `channel_cal_factors` through `analysis.py`** — `calculate_fft`, `calculate_cross_spectrum_matrix`, `calculate_tf`, `calculate_sonogram` all work on raw `time_data` and construct their output (`FreqData`, `TfData`, `SonoData`) with default `channel_cal_factors = ones(...)`. Plotting and modal fitting then multiply by the *new* defaults, so any per-channel calibration set on the source `TimeData` is silently dropped during analysis. Either apply cal factors to time data before computation, or carry them through as metadata into the derived objects (need to pick a convention for TF — output channel's cal? input/output ratio? — before implementing). Affects every cross-channel computation, not just IEPE work.
...
- [x] **IEPE warmup / settling delay** — done. `Recorder_NI_nidaqmx._build_and_start_ai_task` now blocks for `_IEPE_WARMUP_S = 2 s` (~6 RC time constants on the 9234's ~0.32 s AC-coupling HPF) after `task.start()` whenever any channel has IEPE excitation > 0. Adds a one-line "settling" status print so the brief pause isn't surprising. Could be smarter later — currently re-warms on every `init_stream`, even when the previous task was just torn down a moment ago and the AC-couple cap is still settled. A "skip warmup if last teardown was recent" optimisation would shave ~2 s off back-to-back captures, but the simple version is good enough for the typical CLI flow.

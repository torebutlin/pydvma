...
- [ ] **Set up a test suite** — partially done. `tests/` exists with hardware-side coverage (`test_acquisition_*`, `test_ni_*`, `test_oscilloscope_smoke`) and analysis-side coverage (`test_analysis.py`, 20 golden tests for `calculate_fft`, `calculate_cross_spectrum_matrix`, `calculate_tf`). Still to do: `modal.py` (fits on synthetic modes), `datastructure.py` (calibration, list ops), `file.py` (round-trip save/load, CSV, MATLAB), and a mocked `streams.py` harness.

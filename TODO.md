...
- [ ] **IEPE warmup / settling delay** — the 9234's AC coupling has a ~0.3 s RC time constant; the first capture after enabling excitation contains the bias settling transient (mean drifting from ~12 V to 0). Currently `log_data` rebuilds the stream every call, so every fresh acquisition with IEPE on has this transient. Either expose an `iepe_warmup_s` setting that pads the initial sleep, or stop rebuilding the stream when settings haven't changed in a way that matters, or do an internal throwaway capture when IEPE is freshly enabled.

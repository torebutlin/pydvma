...
#: The multisine int-valued spec fields: DERIVED from the keyset above
#: (everything except ``type``, which is a string, and ``amp``, which is
#: a float mapped onto ``amp_rms``) rather than hand-listed, so adding a
#: key in one place cannot leave the two out of step -- the same
#: derive-don't-duplicate rule as ``_SETTINGS_WHITELIST``.
_MULTISINE_INT_KEYS = tuple(sorted(
    _OUTPUT_SPEC_KEYS_MULTISINE - {'type', 'amp'}))
...
def _coerce_multisine_int(output_spec, key):
    ...
    a number.  **Numeric strings are rejected too** (``'64'`` is an
    error, not 64) -- unlike the classic sweep/gaussian branch, which
    coerces its fields with ``float(...)`` and so accepts them.  The
    multisine spec is machine-generated by the BLA run store, never
    hand-typed, so a string here means a caller bug worth surfacing
    rather than papering over.  Raises ``ValueError`` naming the key and
...
def _coerce_multisine_amp(output_spec):
    ...
    Rejects ``bool``, numeric strings (see `_coerce_multisine_int` --
    this branch does not coerce) and, importantly, non-finite values:
    bare JSON has no NaN/Infinity, but a permissive encoder can emit
    them, and the generator's peak guard is a ``>`` comparison that is
    False for NaN -- so a NaN amplitude would sail past every check and
    reach the DAC as a silent all-NaN buffer.
    """
    ...
    if not math.isfinite(value):
        raise ValueError(
            "multisine output key 'amp' must be finite (got %r)"
            % (value,))
    return float(value)
...
def _build_multisine_output_signal(settings, output_spec):
    ...
    There is deliberately no Nyquist check like the classic branch's:
    the multisine has no ``f1``/``f2``, and its only frequency bound is
    the generator's ``k2 <= (N-1)//2``, which IS the output Nyquist for
    a period of ``n_samples`` at ``output_fs``.  The stronger condition
    a BLA run needs -- ``output_fs == fs``, so the excited bins land on
    the same grid the analysis reads -- is a run-level property this
    single-capture builder cannot see; the webui BLA run preflight
    enforces it before the first capture is requested.
    """
    ...
    if unknown:
        # Name the branch: the two keysets barely overlap, so "unknown
        # output key 'f1'" alone reads like a typo when the real cause is
        # a classic-type key sent to a multisine spec.
        raise ValueError(
            'unknown output key(s) for multisine spec: %s. Allowed: %s'
            % (', '.join(sorted(map(str, unknown))),
               ', '.join(sorted(_OUTPUT_SPEC_KEYS_MULTISINE))))
    missing = (_OUTPUT_SPEC_KEYS_MULTISINE - {'type'}) - set(output_spec)
    if missing:
        # ALL of them: a half-built spec is usually missing several, and
        # fixing them one round-trip at a time is needless friction.
        raise ValueError(
            'multisine output spec missing required key(s): %s'
